...
import OwnedBusiness from "../Home/DetailPage/OwnedBusiness";
import ClaimRequest from "../Home/DetailPage/ClaimRequest";
import FollowedBusiness from "../Home/DetailPage/FollowedBusiness";
import AppliedJobs from "../Home/DetailPage/AppliedJobs";
import EventsActivity from "../Home/DetailPage/EventsActivity";
...
const Profile = () => {
  ...
  const tabs = [
    ...
    { id: "active_business", label: "Owned Business" },
    { id: "claim_business", label: "Claim Business" },
    { id: "followed_business", label: "Followed Business" },
    { id: "applied_jobs", label: "Applied Jobs" },
    { id: "events_activity", label: "Events Activity" },
  ];
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "active_business":
        return <OwnedBusiness user={user} />;
      case "claim_business":
        return <ClaimRequest user={user} />;
      case "followed_business":
        return <FollowedBusiness user={user} />;
      case "applied_jobs":
        return <AppliedJobs user={user} />;
        case "events_activity":
        return <EventsActivity user={user} />;
      default:
        return <AboutTab user={user} />;
    }
  };
  ...
  return (
    <div>
      <main>
        <div className="container mx-auto bg-white shadow-md rounded-lg overflow-hidden my-5">
          <CoverPhoto user={user} Update={Update} />

          <div className="px-6 pt-16 pb-6">
            <div className="flex flex-wrap justify-between gap-3 items-center">
              <div>
                <h1 className="text-2xl font-semibold text-gray-800 ">
                  {user?.first_name || "Business Name"}
                </h1>
              </div>
              ...
              <nav className="flex space-x-4 text-gray-600">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`${
                      activeTab === tab.id
                        ? `text-[${theme_content?.button_bg}] border-b-2 border-[${theme_content?.button_bg}] pb-1`
                        : `hover:text-[${theme_content?.button_bg}]`
                    } text-sm font-medium`}
