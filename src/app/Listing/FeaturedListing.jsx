...
const FeaturedListing = () => {
  ...
  const getBusinesses = async () => {
    ...
    setAllBusinesses(response.data.filter(item => item.plan_type == 'premium'))
    setBusinesses(response.data.filter(item => item.plan_type == 'premium'))
    setLoader(false)
  }

  // Filtering function based on selected category or sub-category
  const BusinessFilter = data => {
    // If no filter is applied, show all businesses.
    if (!selctedItems || selctedItems.length === 0) {
      setBusinesses(data)
      return
    }

    // Extract tags from the selected sub-categories.
    const selectedTags = selctedItems.flatMap(item => item.tags || [])
    // If filter is active but no tags are available, then return an empty list.
    if (selectedTags.length === 0) {
      setBusinesses([])
      return
    }
    // Filter businesses that have at least one tag matching any selected tag.
    const filtered = data.filter(business => {
      if (!business.business_tags || business.business_tags.length === 0)
        return false
      return business.business_tags.some(bTag =>
        ...
        <>
          ...
          <div className='w-[90%] lg:w-[1170px] mx-auto py-[60px]'>
            <div className='flex flex-col pb-[60px]'>
              <h3 className='text-[30px] font-semibold'>See how we can help</h3>
              <p className='text-sm'>Home / Listings</p>
            </div>
